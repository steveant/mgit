...
class RepositoryResult:
    ...
    def __init__(self, repo: Repository, org_name: str, project_name: Optional[str] = None):
        self.repo = repo
...
async def list_repositories(
    ...
    logger.debug(f"Parsed query '{query}' into: org={pattern.org_pattern}, project={pattern.project_pattern}, repo={pattern.repo_pattern}")
    ...
    try:
        ...
        if not provider:
            raise MgitError(f"No provider available. Use 'mgit config list' to see configured providers.")
    ...
    try:
        with Progress(
            ...
            discovery_task = progress.add_task("Discovering organizations...", total=None, repos_found=0)
            ...
            logger.debug(f"Found {len(matching_orgs)} matching organizations out of {len(organizations)}")
            
            if not matching_orgs:
                progress.update(discovery_task, completed=True)
                console.print(f"[yellow]No organizations match pattern '{pattern.org_pattern}'[/yellow]")
                return results
            
            # Update overall progress with known organization count
            progress.update(discovery_task, total=len(matching_orgs), completed=0, 
            ...
            for i, org in enumerate(matching_orgs):
                ...
                progress.update(discovery_task, completed=i, repos_found=len(results),
                              description=f"Processing {org.name} ({i+1}/{len(matching_orgs)})")
                
                # Add organization-specific task
                org_task = progress.add_task(f"  └─ Scanning {org.name}", total=None, repos_found=len(results))
                ...
                try:
                    ...
                    if provider.supports_projects():
                        ...
                        if not matching_projects and pattern.has_project_filter:
                            progress.update(org_task, description=f"  └─ {org.name}: No matching projects", completed=True)
                            continue
                        
                        # Update with project count if we have projects to process
                        if matching_projects:
                            progress.update(org_task, total=len(matching_projects), completed=0,
                            ...
                            for j, project in enumerate(matching_projects):
                                ...
                                project_task = progress.add_task(f"      └─ {project.name}", total=None, repos_found=0)
                                project_repos = 0
                                
                                async for repo in provider.list_repositories(org.name, project_name):
                                    if matches_pattern(repo.name, pattern.repo_pattern):
                                        results.append(RepositoryResult(repo, org.name, project_name))
                                        project_repos += 1
                                        
                                        # Update counters
                                        progress.update(project_task, repos_found=project_repos,
                                                      description=f"      └─ {project.name}: {project_repos} repos")
                                        progress.update(org_task, repos_found=len(results))
                                        progress.update(discovery_task, repos_found=len(results))
                        ...
                        async for repo in provider.list_repositories(org.name):
                            if matches_pattern(repo.name, pattern.repo_pattern):
                                ...
                                # Update counters
                                progress.update(org_task, repos_found=org_repos,
                    ...
                    progress.update(org_task, description=f"  └─ {org.name}: Error - {str(e)[:50]}", completed=True)
                    continue
            
            # Final update
            progress.update(discovery_task, completed=len(matching_orgs), repos_found=len(results),
