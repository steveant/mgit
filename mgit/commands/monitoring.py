...
def start_monitoring_server_command(
    ...
    logger.info(
        f"Starting mgit monitoring server on {host}:{port}",
        host=host,
        port=port,
        simple=simple,
    )
...
def health_check_command(
    detailed: bool = False, json_output: bool = False, exit_code: bool = True
) -> None:
...
if TYPER_AVAILABLE:
    ...
    monitoring_app = typer.Typer(
        name="monitoring", help="Monitoring and observability commands for mgit"
    )
    ...
    def health_cmd(
        ...
        no_exit: bool = typer.Option(
            False, "--no-exit", help="Don't exit with status code"
        ),
    ):
        """Run health checks."""
