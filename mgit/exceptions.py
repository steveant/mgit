...
def retry_with_backoff(
    ...
    exceptions: tuple[Type[Exception], ...] = (ConnectionError, RepositoryOperationError),
                ...
                try:
                    ...
                    if on_retry:
                        ...
                        logger.warning(
                            ...
                            attempt + 1, retries + 1, str(e), current_delay
                        )
...
def validate_url(url: str, provider: Optional[str] = None) -> None:
    ...
    if provider == "azuredevops" and "dev.azure.com" not in url and "visualstudio.com" not in url:
        raise ValidationError(
