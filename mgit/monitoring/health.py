...
class HealthChecker:
    ...
    def _register_default_checks(self) -> None:
        ...
        self.register_check(
            "git_availability", self._check_git_availability, interval=60
        )
        self.register_check(
            "network_connectivity", self._check_network_connectivity, interval=30
        )
        ...
        self.register_check(
            "provider_endpoints", self._check_provider_endpoints, interval=120
        )
        self.register_check(
            "authentication_status", self._check_authentication_status, interval=300
        )
    ...
    async def run_check(self, name: str) -> HealthCheckResult:
        ...
        if name not in self._checks:
            return HealthCheckResult(
                name=name, status="unknown", message=f"Health check '{name}' not found"
            )
        ...
        try:
            ...
            # Record metrics
            self.metrics.set_gauge(
                "mgit_health_check_status", 0.0, labels={"check": name}
            )
    ...
    async def get_overall_health(self, use_cache: bool = True) -> Dict[str, Any]:
        ...
        if (
            use_cache
            and self._overall_health_cache
        ...
        unhealthy_checks = [
            name for name, r in results.items() if r.status != "healthy"
        ]
        ...
        self.metrics.set_gauge(
            "mgit_health_overall", 1.0 if overall_status == "healthy" else 0.0
        )
        self.metrics.set_gauge(
            "mgit_health_percentage", health_data["summary"]["health_percentage"]
        )
    ...
    async def is_ready(self) -> bool:
        ...
        for check_name in critical_checks:
            ...
            if result.status != "healthy":
                self.logger.warning(
                    f"Readiness check failed: {check_name}",
    ...
    async def _check_provider_endpoints(self) -> HealthCheckResult:
        ...
        accessible_count = sum(
            1 for status in results.values() if status == "accessible"
        )
    ...
    async def _check_authentication_status(self) -> HealthCheckResult:
        ...
        try:
            ...
            if providers_configured == 0:
                ...
                status = "healthy"
                message = (
                    f"Authentication configured for {providers_configured} providers"
                )
