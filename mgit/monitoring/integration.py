        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            with correlation_context(operation=actual_operation_name, **context_data):
                ...
                try:
                    ...
                    if log_result:
                        logger.operation_end(actual_operation_name, success=True, duration=duration)
                    ...
                    # End performance tracking
                    if track_performance and operation_id:
                        performance_monitor.end_trace(operation_id, success=False, error=str(e))
                    
                    # Log operation failure
                    if log_result:
                        logger.operation_end(actual_operation_name, success=False, duration=duration)
                        logger.error(f"Operation {actual_operation_name} failed: {str(e)}",
...
def monitor_git_operation(
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args, **kwargs) -> T:
            ...
            if not repository:
                repository = kwargs.get('repository') or kwargs.get('repo_url') or kwargs.get('url')
            ...
            with git_operation_context(actual_operation_type, repository=repository):
                ...
                try:
                    # Log operation start
                    logger.operation_start(f"git_{actual_operation_type}", repository=repository)
                    ...
                    # Log operation success
                    logger.git_operation(actual_operation_type, repository or "unknown", success=True)
                    ...
                    # Log operation failure
                    logger.git_operation(actual_operation_type, repository or "unknown", success=False)
                    logger.error(f"Git {actual_operation_type} failed: {str(e)}",
                               error=str(e), error_type=type(e).__name__)
...
class MonitoringContext:
    ...
    def __init__(self, operation_name: str, provider: Optional[str] = None,
                 repository: Optional[str] = None, **metadata):
...
def setup_monitoring_integration():
    ...
    logger.info("Monitoring integration initialized",
                components=["structured_logging", "metrics", "health_checks", "performance_monitoring"])