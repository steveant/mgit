...
class PerformanceMonitor:
    ...
    def start_trace(self, operation_name: str, operation_id: Optional[str] = None,
        ...
        self.metrics.inc_counter("mgit_performance_operations_started_total",
                                labels={"operation": operation_name})
        
        self.logger.debug(f"Started performance trace: {operation_name}",
                         operation_id=operation_id,
                         operation_name=operation_name,
                         parent_id=parent_id)
    ...
    def end_trace(self, operation_id: str, success: bool = True,
        ...
        with self.lock:
            ...
            if not trace:
                self.logger.warning(f"Trace not found: {operation_id}",
                                  operation_id=operation_id)
                return None
        ...
        if (hasattr(self._operation_stack, 'stack') and 
            self._operation_stack.stack and 
            self._operation_stack.stack[-1] == operation_id):
            self._operation_stack.stack.pop()
        
        # Record metrics
        operation_name = trace.operation_name
        self.metrics.observe_histogram("mgit_performance_operation_duration_seconds",
                                     trace.duration,
                                     labels={"operation": operation_name, "success": str(success)})
        
        self.metrics.inc_counter("mgit_performance_operations_completed_total",
                               labels={"operation": operation_name, "success": str(success)})
        
        if not success:
            self.metrics.inc_counter("mgit_performance_operations_errors_total",
                                   labels={"operation": operation_name})
        
        self.logger.info(f"Completed performance trace: {operation_name}",
                        operation_id=operation_id,
                        operation_name=operation_name,
    ...
    def get_operation_metrics(self, operation_name: str, 
                            hours: int = 24) -> Optional[PerformanceMetrics]:
        ...
        with self.lock:
            ...
            recent_traces = [
                ...
                if (trace.operation_name == operation_name and 
                    trace.start_time > cutoff_time and 
    ...
    def get_all_operations_summary(self, hours: int = 24) -> Dict[str, PerformanceMetrics]:
    ...
    def set_performance_baseline(self, operation_name: str, 
        ...
        self.logger.info(f"Set performance baseline for {operation_name}",
                        operation_name=operation_name,
    ...
    def _check_performance_anomalies(self, trace: PerformanceTrace) -> None:
        ...
        if baseline and trace.duration:
            ...
            if baseline_avg > 0 and trace.duration > baseline_avg * 3:
                self.logger.warning(f"Performance anomaly: {operation_name} took {trace.duration:.2f}s (baseline avg: {baseline_avg:.2f}s)",
                ...
                self.metrics.inc_counter("mgit_performance_anomalies_total",
                                       labels={"operation": operation_name, "type": "slow_operation"})
            
            elif baseline_p95 > 0 and trace.duration > baseline_p95 * 2:
                self.logger.warning(f"Performance anomaly: {operation_name} exceeded P95 baseline",
                                   operation_id=trace.operation_id,
                                   operation_name=operation_name,
                                   duration=trace.duration,
            ...
            for operation_name in list(self._operation_metrics.keys()):
                ...
                if len(self._operation_metrics[operation_name]) > 1000:
                    self._operation_metrics[operation_name] = \
                        self._operation_metrics[operation_name][-500:]
    ...
    @asynccontextmanager
    async def async_trace_operation(self, operation_name: str,
                                   tags: Optional[Dict[str, str]] = None,
                                   metadata: Optional[Dict[str, Any]] = None):
