...
class MonitoringServer:
    ...
    async def start(self) -> None:
        ...
        if not AIOHTTP_AVAILABLE:
            raise RuntimeError("aiohttp is required for the monitoring server. Install with: pip install aiohttp")
    ...
    async def _request_middleware(self, request: Request, handler: Callable) -> Response:
        ...
        # Log request
        self.logger.info(f"{method} {path} -> {status_code}",
                        method=method,
                        path=path,
                        status_code=status_code,
    ...
    async def _status_handler(self, request: Request) -> Response:
        ...
        try:
            is_healthy = (await self.health_checker.get_overall_health())["status"] == "healthy"
    ...
    def start(self) -> None:
        ...
        class MonitoringRequestHandler(BaseHTTPRequestHandler):
            ...
            def _handle_readiness(self):
                self._send_json_response(200, {"status": "ready", "timestamp": datetime.now().isoformat()})
            
            def _handle_liveness(self):
                self._send_json_response(200, {"status": "alive", "timestamp": datetime.now().isoformat()})
...
async def start_monitoring_server(host: str = "0.0.0.0", port: int = 8080) -> MonitoringServer:
    """Start the monitoring server.
...
def start_simple_monitoring_server(host: str = "0.0.0.0", port: int = 8080) -> SimpleMonitoringServer:
