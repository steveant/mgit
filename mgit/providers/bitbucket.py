...
class BitBucketProvider(GitProvider):
    ...
    def _validate_config(self) -> None:
        ...
        if not self._validator.validate_organization_name(self.workspace):
            self._monitor.log_validation_failure('bitbucket_workspace', self.workspace, 'Invalid workspace name format')
            raise ConfigurationError("Invalid BitBucket workspace name format")
            
        # Validate base URL
        if not self._validator.validate_url(self.base_url):
            raise ConfigurationError(f"Invalid or insecure base_url: {mask_sensitive_data(self.base_url)}")
            
        if self.auth_method == 'app_password':
            if not self.username:
                raise ConfigurationError("Username is required for app_password authentication")
            if not self.app_password:
                raise ConfigurationError("App password is required for app_password authentication")
            # Validate app password format
            if not validate_bitbucket_app_password(self.app_password):
                self._monitor.log_validation_failure('bitbucket_app_password', mask_sensitive_data(self.app_password), 'Invalid app password format')
                raise ConfigurationError("Invalid BitBucket app password format")
        elif self.auth_method == 'oauth':
            if not self.oauth_token:
                raise ConfigurationError("OAuth token is required for oauth authentication")
        else:
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            async with self._session.get(f"{self.base_url}/user", headers=headers) as response:
                if response.status == 200:
                    ...
                    raise AuthenticationError(f"Authentication failed with status {response.status}: {text}")
        ...
        try:
            ...
            while url:
                async with self._session.get(url, headers=headers) as response:
                    if response.status != 200:
                        text = await response.text()
                        raise APIError(f"Failed to list workspaces: {response.status} - {text}", self.PROVIDER_NAME)
                    ...
                    for workspace_data in data.get('values', []):
                        organizations.append(Organization(
                            name=workspace_data.get('slug', workspace_data.get('name', '')),
                            url=workspace_data.get('links', {}).get('html', {}).get('href', ''),
        ...
        try:
            ...
            while url:
                async with self._session.get(url, headers=headers) as response:
                    if response.status != 200:
                        text = await response.text()
                        raise APIError(f"Failed to list projects: {response.status} - {text}", self.PROVIDER_NAME)
    ...
    async def list_repositories(
        ...
        if filters:
            ...
            if 'is_private' in filters:
                private_filter = 'is_private=true' if filters['is_private'] else 'is_private=false'
                if 'q' in params:
        ...
        try:
            ...
            while url:
                async with self._session.get(url, headers=headers, params=params) as response:
                    if response.status != 200:
                        text = await response.text()
                        raise APIError(f"Failed to list repositories: {response.status} - {text}", self.PROVIDER_NAME)
        ...
        try:
            async with self._session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    raise PermissionError(f"Access denied to repository {organization}/{repository}", self.PROVIDER_NAME)
                else:
                    text = await response.text()
                    raise APIError(f"Failed to get repository: {response.status} - {text}", self.PROVIDER_NAME)
    ...
    def get_authenticated_clone_url(self, repository: Repository) -> str:
        ...
        if self.auth_method == 'app_password':
            ...
            if repository.clone_url.startswith('https://bitbucket.org/'):
                ...
                return f"https://{encoded_token}:x-oauth-basic@bitbucket.org/{repo_path}"
