...
class ProviderManager:
    ...
    def _resolve_provider(self) -> None:
        ...
        try:
            if self.provider_name:
                ...
                logger.debug(f"Using named provider '{self.provider_name}' of type '{self._provider_type}'")
                
            elif self.auto_detect_url:
                # Auto-detect provider type from URL and use default config of that type
                self._provider_type = self._detect_provider_from_url(self.auto_detect_url)
                self._config = self._find_config_by_type(self._provider_type)
                logger.debug(f"Auto-detected provider type '{self._provider_type}' from URL")
                ...
                if default_name:
                    ...
                    logger.debug(f"Using default provider '{default_name}' of type '{self._provider_type}'")
                else:
        ...
        for name, config in providers.items():
            try:
                if detect_provider_type(name) == provider_type:
                    ...
                    logger.debug(f"Found '{name}' configuration for provider type '{provider_type}'")
                    return config
            except ValueError:
                continue
        
        raise ConfigurationError(f"No configuration found for provider type '{provider_type}'")
    ...
    def get_provider(self) -> GitProvider:
        ...
        try:
            self._provider = ProviderFactory.create_provider(self._provider_type, self._config)
            logger.debug(f"Created {self._provider_type} provider from config '{self.provider_name}'")
            return self._provider
            
        except Exception as e:
            raise ProviderNotFoundError(f"Failed to create {self._provider_type} provider: {e}")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning("test_connection called from async context - use test_connection_async instead")
    ...
    async def list_repositories_async(self, project: str):
        ...
        try:
            ...
            raise ProviderNotFoundError(f"No suitable provider available for {self._provider_type}: {e}")
    ...
    def list_repositories(self, project: str):
        ...
        try:
            ...
            try:
                loop = asyncio.get_running_loop()
                # We're in an async context
                logger.warning("list_repositories called from async context - use list_repositories_async instead")
                # Create a new thread to run the async code
                import concurrent.futures
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(asyncio.run, self.list_repositories_async(project))
                    return future.result()
            except RuntimeError:
                # Not in an async context, safe to use asyncio.run
