...
class ProviderRegistry:
    ...
    URL_PATTERNS: List[Tuple[re.Pattern, str]] = [
        ...
        (re.compile(r"https?://[^/]+\.visualstudio\.com", re.IGNORECASE), "azuredevops"),
        (re.compile(r"https?://[^/]+/tfs", re.IGNORECASE), "azuredevops"),  # On-prem TFS
        
        # GitHub patterns
        (re.compile(r"https?://github\.com/[^/]+", re.IGNORECASE), "github"),
        (re.compile(r"https?://[^/]+/.*github", re.IGNORECASE), "github"),  # GitHub Enterprise
        ...
        (re.compile(r"https?://[^/]+/.*bitbucket", re.IGNORECASE), "bitbucket"),  # Bitbucket Server
    ...
    def _validate_provider_class(self, name: str, provider_class: Type[GitProvider]) -> None:
        ...
        required_attrs = ["PROVIDER_NAME", "SUPPORTED_AUTH_METHODS", "DEFAULT_API_VERSION"]
        ...
        abstract_methods = [
            "_validate_config", "authenticate", "test_connection",
            "list_organizations", "list_projects", "list_repositories",
            "get_repository", "get_authenticated_clone_url"
        ]
    ...
    def get_provider_by_url(self, url: str, config: Optional[Dict[str, Any]] = None) -> GitProvider:
    ...
    def auto_discover(self, package_path: Optional[Path] = None) -> None:
        ...
        for py_file in package_path.glob("*.py"):
            # Skip special files
            if py_file.name.startswith("_") or py_file.stem in ["base", "registry", "factory", "exceptions"]:
                continue
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if (issubclass(obj, GitProvider) and 
                        obj is not GitProvider and
    ...
    def get_provider_info(self, provider_type: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_auth_methods": [m.value for m in provider_class.SUPPORTED_AUTH_METHODS],
            "default_api_version": provider_class.DEFAULT_API_VERSION,
            "supports_projects": provider_class().supports_projects() if hasattr(provider_class(), "supports_projects") else True,
