...
class SecurityConfig:
        ...
        for key in self.settings:
            ...
            if env_value is not None:
                ...
                if isinstance(self.settings[key], bool):
                    ...
                    self.settings[key] = [item.strip() for item in env_value.split(',') if item.strip()]
                else:
    ...
    def _validate_settings(self):
        ...
        if self.settings['rate_limit_requests_per_minute'] <= 0:
            ...
            self.settings['rate_limit_requests_per_minute'] = self.DEFAULT_SETTINGS['rate_limit_requests_per_minute']
        
        # Validate log level
        valid_log_levels = ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']
        if self.settings['log_level'].upper() not in valid_log_levels:
            logger.warning(f"Invalid log_level: {self.settings['log_level']}, using INFO")
        ...
        if self.settings['debug_mode']:
            logger.warning("SECURITY: Debug mode is enabled - may expose sensitive information!")
        
        if self.settings['detailed_error_messages']:
            logger.warning("SECURITY: Detailed error messages enabled - may expose sensitive information!")
    ...
    def is_production_secure(self) -> bool:
        ...
        for key, required_value in required_settings.items():
            if self.settings.get(key) != required_value:
                issues.append(f"{key} should be {required_value}, got {self.settings.get(key)}")
...
def init_security_config(config_file: Optional[Union[str, Path]] = None) -> SecurityConfig:
