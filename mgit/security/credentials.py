...
class CredentialMasker:
    ...
    SENSITIVE_FIELDS = {
        'pat', 'token', 'password', 'secret', 'key', 'auth', 'credential',
        'app_password', 'oauth_token', 'access_token', 'refresh_token',
        'authorization', 'x-api-key', 'x-auth-token'
    }
    
    # URL parameters that should be masked
    SENSITIVE_URL_PARAMS = {
        'token', 'access_token', 'auth', 'key', 'secret', 'pat'
    }
    ...
    def mask_url(self, url: str) -> str:
        ...
        try:
            ...
            if parsed.username or parsed.password:
                masked_username = self._mask_credential(parsed.username) if parsed.username else ''
                masked_password = self._mask_credential(parsed.password) if parsed.password else ''
    ...
    def mask_headers(self, headers: Dict[str, str]) -> Dict[str, str]:
        ...
        for key, value in headers.items():
            ...
            if any(sensitive in key_lower for sensitive in self.SENSITIVE_FIELDS):
                ...
                masked_headers[key] = self.mask_string(value) if isinstance(value, str) else value
