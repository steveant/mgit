...
class SecurityIntegration:
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'failed_auth_attempts': monitor_summary.get('metrics', {}).get('failed_auth_attempts', 0),
            'validation_failures': monitor_summary.get('metrics', {}).get('validation_failures', 0),
            'security_events': monitor_summary.get('total_events', 0),
            'recommendations': monitor_summary.get('recommendations', []),
            'configuration': {
                'credential_masking': self.security_config.get('mask_credentials_in_logs'),
...
def create_security_cli_commands():
    ...
    def init_security_config(
        config_file: Optional[str] = typer.Option(None, help="Security configuration file")
    ):
        """Initialize security configuration."""
