...
class SecurityLogger:
    ...
    def log_api_call(self, method: str, url: str, status_code: Optional[int] = None, 
        ...
        if status_code and response_time:
            self.info(f"API {method} {masked_url} -> {status_code} ({response_time:.2f}s)")
        else:
...
def log_credential_exposure_attempt(logger: logging.Logger, context: str, data: str):
    ...
    logger.warning(f"SECURITY: Potential credential exposure in {context}: {masked_data}")


def log_validation_failure(logger: logging.Logger, input_type: str, value: str, reason: str):
    ...
    if any(keyword in input_type.lower() for keyword in ['token', 'password', 'auth', 'secret']):
        masker = CredentialMasker()
        value = masker.mask_string(value)
    
    logger.warning(f"SECURITY: Input validation failed for {input_type}: {value} - {reason}")
...
def log_suspicious_activity(logger: logging.Logger, activity: str, details: Dict[str, Any]):
    ...
    logger.warning(f"SECURITY: Suspicious activity detected - {activity}: {masked_details}")