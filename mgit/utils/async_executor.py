...
class AsyncExecutor:
    ...
    def __init__(self, concurrency: int = 4, mode: ExecutionMode = ExecutionMode.CONCURRENT):
        """
        ...
        self.mode = mode
        self.semaphore = asyncio.Semaphore(concurrency) if mode == ExecutionMode.CONCURRENT else None
        ...
        if not show_progress:
            ...
            if self.mode == ExecutionMode.CONCURRENT:
                tasks = [
                    self._process_item_with_semaphore(
                        idx, item, process_func, results, errors, 
                        collect_errors, on_error, on_success
                    )
            ...
            with Progress() as progress:
                ...
                for idx, item in enumerate(items):
                    ...
                    task_id = progress.add_task(f"[grey50]Pending: {desc}[/grey50]", total=1)
                ...
                if self.mode == ExecutionMode.CONCURRENT:
                    tasks = [
                        self._process_item_with_progress(
                            idx, item, process_func, results, errors,
                            progress, overall_task, item_tasks[idx],
                            item_description, collect_errors, on_error, on_success
                        )
                        for idx, item in enumerate(items)
                    ]
                    await asyncio.gather(*tasks)
        ...
        if self.mode == ExecutionMode.CONCURRENT and self.semaphore:
            async with self.semaphore:
                await self._process_with_progress_update(
                    idx, item, process_func, results, errors,
                    progress, overall_task, item_task, desc,
                    collect_errors, on_error, on_success
                )
        else:
            await self._process_with_progress_update(
    ...
    async def _process_with_progress_update(
        ...
        progress.update(item_task, description=f"[cyan]Processing: {desc}...", visible=True)
        
        try:
            result = await process_func(item)
            results[idx] = result
            progress.update(item_task, description=f"[green]✓ Completed: {desc}[/green]", completed=1)
            if on_success:
                on_success(item, result)
        except Exception as e:
            logger.warning(f"Error processing {desc}: {e}")
            errors.append((item, e))
            progress.update(item_task, description=f"[red]✗ Failed: {desc}[/red]", completed=1)
            if on_error:
                on_error(item, e)
            if not collect_errors:
...
class SubprocessExecutor(AsyncExecutor):
    ...
    async def run_commands(
        ...
        show_progress: bool = True,
    ) -> Tuple[List[Tuple[int, bytes, bytes]], List[Tuple[Tuple[str, List[str], Path], Exception]]]:
        """
        ...
        """
        async def run_command(cmd_info: Tuple[str, List[str], Path]) -> Tuple[int, bytes, bytes]:
